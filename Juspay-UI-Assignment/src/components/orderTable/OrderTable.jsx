...
import Person12 from "../rightPanel/Images/person12.svg";
...
const OrdersTable = () => {
  ...
  const handleAddRow = () => {
    ...
    setNewRow({ id: "", user: "", project: "", address: "", date: "", status: "" });
    handleCloseModal();
  };
  ...
  return (
    <DataContainer isDarkMode={isDarkMode}>
      <h4 style={{ color: isDarkMode ? "white" : "black" }}>Order List</h4>
      <Paper
        elevation={3}
        sx={{ borderRadius: "12px", overflow: "hidden", bgcolor: isDarkMode ? "#1e1e1e" : "#FFFFFF" }}
      >
        <Toolbar sx={{ justifyContent: "space-between", bgcolor: isDarkMode ? "#FFFFFF0D" : "#F7F9FB" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <IconButton {...headerGroups[0].headers[1].getSortByToggleProps()}>
              <SortByAlphaTwoTone sx={{ color: isDarkMode ? "white" : "black" }} />
            </IconButton>
          </Box>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <TextField
        ...
        {isFilterOpen && (
          <Box sx={{ p: 2, backgroundColor: isDarkMode ? "#2a2a2a" : "#F7F9FB" }}>
            <Typography variant="body2" mb={1} sx={{ color: isDarkMode ? "white" : "black" }}>
              Filter by Project:
            </Typography>
            <TextField
              fullWidth
              ...
              <TableRow>
                <TableCell padding="checkbox">
                  <Checkbox
                    ...
                    checked={pagedRows.length > 0 && pagedRows.every((row) => selectedRows[row.original.id])}
                    indeterminate={
